            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border border-[var(--outline)]/20 overflow-hidden shadow-xl",
          ...
          {items.map((it, idx) => (
            <button
              ...
              className={cn(
                ...
                idx === active
                  ? "w-6 bg-[var(--primary)]"
                  : "w-2 bg-[var(--outline)]/30 hover:bg-[var(--outline)]/60",
              )}
...
function DefaultCard({ item, active: _active }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full bg-[var(--surface-container)] p-6 flex flex-col justify-end">
      {item.imageSrc ? (
        <>
